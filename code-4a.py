""" Code to solve aoc-2021 day 4 part a """
...
def initialise_bingo_counter(input_path):
    ...
    for i in range(board_count):
        bingo_counter.append([0 for n in range(10)])
...
def main(input_path):
    """ Plays game and returns final score """
    ...
    number_list = read_numbers(input_path)
    # Set bingo status
    bingo = False
    ...
    for n in number_list:
        ...
        for j in range(len(board_list)):
            ...
            if max(bingo_counter[j]) == 5:
                bingo = True
                win_board = board_list[j]
                win_n = n
                break
        if bingo == True:
            break
    return calculate_score(number_list, win_n, win_board)
